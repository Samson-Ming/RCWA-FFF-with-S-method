...
%permW = 1.72^2;
%permW = 3^2;
%permW  = 1.5^2;   %NOTE that the permitivity is complex!!!
%permW  = (1+5i)^2; 
permW  = (0.2+6i)^2;   %NOTE that the permitivity is complex!!!
...
%fsin=@(t) (sec(Phi)+sin(Phi)*d_bt/d_t)/norm;
%fcos=@(t) (cos(Phi)*d_bt/d_t)/norm;
...
%d_bt/d_t=((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t))));

fsin=@(t) (sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))/...
          sqrt((sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2+...
          (cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2);

fcos=@(t) (cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))/...
          sqrt((sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2+...
          (cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2);
...
%fcos=C^2=fsin^2_above; fsin=C*S=fsin*fcos_above
...
fcos=@(t) (sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2/...
          ((sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2+...
          (cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2);

fsin=@(t) (sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))*(cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))/...
          ((sec(Phi)+sin(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2+...
          (cos(Phi)*((K1*(d/cos(Phi))/2)*(-Beta*(1-sin(K1/2*t)^(2*Alpha))^(Beta-1)*(-2*Alpha*sin(K1/2*t)^(2*Alpha-1)*cos(K1/2*t)))))^2);
...
%RP_ref=[0.0359515331519222;0.00206378125825475;0.00451976465041257;0.00213946228010985]';
RP_ref=[0.0316061412428994;0.506963318688133;0.0362891836744555;0.104311142457780]';
...
N_l    = 1000;           % lower number of layers
N_u    = 1000;           % upper number of layers
...
for nMax =  nMax_l:nMax_step:nMax_u
    ...
    for k=1:N
    %t(1,k)=acos(-(2*(zV(k))/d-1))/(K1);
    t(1,k)=2/K1*asin((1-(1-zV(k)/d)^(1/Beta))^(1/2/Alpha));
...
%save(filename)
%exit
%save lossy_overhang_Power_sinusodial_FMM_FFF.mat
Compare=[RPvec(:),RP_ref']